...
initalize_mongo_rs=$(echo "rs.initiate({_id: \"rs0\", version: 1, members: [{ _id: 0, host : \"127.0.0.1:27017\" }]}).ok || rs.status().ok" | mongo -u "${MONGO_INITDB_ROOT_USERNAME}" -p "${MONGO_INITDB_ROOT_PASSWORD}" --quiet)
fi
